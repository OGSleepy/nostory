                ...
                <div className="absolute bottom-20 left-4 right-4 z-30 flex gap-2">
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        // Build a minimal NostrEvent from the story — avoids re-querying
                        // which could return a wrong event (kind 30315 vs kind 1 collision)
                        const storyEvent = {
                          id: currentStory.id,
                          pubkey: currentStory.pubkey,
                          kind: currentStory.kind,
                          content: currentStory.content,
                          created_at: currentStory.created_at,
                          tags: currentStory.tags,
                          sig: currentStory.sig,
                        };
                        await publishReply(storyReply.trim(), storyEvent as any);
                        setStoryReply('');
