...
const Nip05Badge: React.FC<{ nip05: string; pubkey: string }> = ({ nip05, pubkey }) => {
  ...
  return <BadgeCheck className="h-4 w-4 text-purple-400 inline-block ml-1 flex-shrink-0" title={`NIP-05: ${nip05}`} />;
};
...
export const PostCard: React.FC<{ post: Post }> = ({ post }) => {
  ...
  const { profiles, pubkey, isAuthenticated, isBookmarked, isMuted } = useNostrStore();
      ...
      <div className="bg-[#1a1a1a] rounded-2xl p-4 space-y-4">
        ...
        <div className="flex items-center justify-between pt-1">
          <div className="flex items-center gap-1">
            ...
            <Button variant="ghost" size="sm" onClick={() => setShowZapDialog(true)} className="text-gray-400 hover:text-yellow-400 hover:bg-yellow-500/10">
