...
export const MobileNav: React.FC = () => {
  const { activeTab, setActiveTab, notifications, notificationsLastReadAt } = useNostrStore();
  const unread = notifications.filter(n => n.created_at > notificationsLastReadAt).length;

  const navItems: Array<{ id: string; icon: React.ElementType; badge?: number; isCenter?: boolean }> = [
    { id: 'feed', icon: Home },
