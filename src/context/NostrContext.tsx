...
interface NostrContextValue {
  ...
  createAccount: () => Promise<{ nsec: string; npub: string } | null>;
  logout: () => void;
...
export const NostrProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Use useState (not useRef) so signer changes trigger re-renders and context consumers update
  const [signerState, setSignerState] = useState<NostrSigner | null>(null);
  const signerRef = useRef<NostrSigner | null>(null);
  // Keep ref in sync with state so callbacks always see the latest signer without stale closures
  const setSigner = (s: NostrSigner | null) => {
    signerRef.current = s;
    setSignerState(s);
  };
  ...
  const loginWithExtension = useCallback(async () => {
    try {
      ...
      return true;
    } catch (error) { console.error('Extension login failed:', error); return false; }
  ...
  const loginWith_nsec = useCallback(async (nsec: string) => {
    try {
      ...
      await postLogin(pubKey);
      return true;
    } catch (error) { console.error('nsec login failed:', error); return false; }
  }, [setAuth, addProfile]);
  ...
  const loginWithBunker = useCallback(async (bunkerUri: string) => {
    try {
      ...
      await postLogin(pubKey);
      return true;
    } catch (error) { console.error('Bunker login failed:', error); return false; }
  }, [setAuth, addProfile, setContacts]);
  ...
  // Generate a brand-new keypair, persist it via AccountManager, and return the nsec for the user to back up
  const createAccount = useCallback(async (): Promise<{ nsec: string; npub: string } | null> => {
    try {
  ...
  // Init AccountManager and restore signer from persisted accounts on mount
  useEffect(() => {
    let sub: { unsubscribe: () => void } | null = null;

    initAccountManager().then(() => {
      const active = accountManager.active;
      if (active) {
        setSigner(active.signer as unknown as NostrSigner);
        // If Zustand store has pubkey but we restored a different account, keep in sync
        const store = useNostrStore.getState();
        if (!store.isAuthenticated) {
          // Accounts restored but store was cleared — restore auth state
          active.getPublicKey().then(pk => {
            setAuth(pk, nip19.npubEncode(pk));
          });
        }
      } else if (useNostrStore.getState().isAuthenticated) {
        // Store thinks we're logged in but no account in manager — clear stale state
        storeLogout();
      }

      // Subscribe to future active-account changes
      sub = accountManager.active$.subscribe(account => {
  ...
  const value: NostrContextValue = {
    ...
    loginWithExtension, loginWith_nsec, loginWithBunker, createAccount, logout,
    signer: signerState,
...
function eventToStory(event: NostrEvent): Story | null {
  try {
    ...
    // Only accept events with a media URL pointing to an image or video
