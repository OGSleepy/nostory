...
export const NostrProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const { pubkey, setAuth, logout: storeLogout, addProfile, addPosts, setContacts, addStories,
    setNwcUrl, setNwcConnected, setWalletBalance, loginMethod,
  } = useNostrStore();
  ...
  const loginWithExtension = useCallback(async () => {
    try {
      ...
      useNostrStore.getState().setLoginMethod('extension');
      sessionStorage.removeItem('nostory_nsec');
  ...
  const loginWith_nsec = useCallback(async (nsec: string) => {
    try {
      ...
      useNostrStore.getState().setLoginMethod('nsec');
      sessionStorage.setItem('nostory_nsec', nsec); // session-only, clears on browser close
      setAuth(pubKey, nip19.npubEncode(pubKey));
      await postLogin(pubKey);
      return true;
  ...
  const loginWithBunker = useCallback(async (bunkerUri: string) => {
    try {
      ...
      useNostrStore.getState().setLoginMethod('bunker');
      setAuth(pubKey, nip19.npubEncode(pubKey));
      await postLogin(pubKey);
      return true;
    } catch (error) {
  ...
  // Auto-restore signer after page refresh (signerRef is in-memory only)
  useEffect(() => {
    const store = useNostrStore.getState();
    if (!store.isAuthenticated || signerRef.current) return;

    const method = store.loginMethod;
    if (method === 'extension') {
      // Re-attach NIP-07 extension signer silently
      if (window.nostr) {
        signerRef.current = window.nostr as NostrSigner;
      } else {
        // Extension not available yet — wait a tick then retry
        setTimeout(() => {
          if (window.nostr) signerRef.current = window.nostr as NostrSigner;
          else store.logout(); // extension truly gone, force re-login
        }, 1500);
      }
    } else if (method === 'nsec') {
      const stored = sessionStorage.getItem('nostory_nsec');
      if (stored) {
        try {
          const { type, data } = nip19.decode(stored);
          if (type === 'nsec') signerRef.current = new NSecSigner(data as Uint8Array);
        } catch { store.logout(); }
      } else {
        // Session ended (tab closed and reopened), force re-login
        store.logout();
      }
    }
    // bunker: can't auto-restore the NIP-46 session, just log out cleanly
