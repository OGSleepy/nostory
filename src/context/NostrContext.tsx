...
import {
  NRelay1, NPool, NSecSigner, NConnectSigner, BlossomUploader,
  NCache, NSet,
...
export const NostrProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Relay list store: keeps NIP-65 events for outbox routing
  const relayStoreRef = useRef<NSet>(new NSet());
  const [isConnected, setIsConnected] = useState(false);
  const { pubkey, setAuth, logout: storeLogout, addProfile, addPosts, setContacts, addStories,
    setNwcUrl, setNwcConnected, setWalletBalance, nwcUrl,
  } = useNostrStore();
      ...
      const pool = new NPool({
        ...
        reqRouter: async (filters) => {
          ...
          if (allAuthors.size > 0) {
            const relayEvents = await relayStoreRef.current.query([{
            ...
            for (const event of relayEvents) {
              const authorUrls = event.tags
                .filter(t => t[0] === 'r' && t[1])
                .map(t => t[1])
                .slice(0, 3); // cap to 3 relays per author for performance
        ...
        eventRouter: async (event) => {
          ...
          if (relayEvents.length > 0) {
            const writeRelays = relayEvents[0].tags
  ...
  const postLogin = async (pubKey: string) => {
    ...
    if (relayListEvents.length > 0) {
      await relayStoreRef.current.add(relayListEvents[0]);
  ...
  const uploadFile = useCallback(async (file: File): Promise<string> => {
    ...
    const uploader = new BlossomUploader({ servers: [BLOSSOM_SERVER], signer: signerRef.current });
    const tags = await uploader.upload(file);
    const urlTag = tags.find(t => t[0] === 'url');
    if (!urlTag?.[1]) throw new Error('Upload failed: no URL returned');
    return urlTag[1];
  }, []);
  ...
  const nwcRef = useRef<{ walletPubkey: string; secret: Uint8Array; relay: string } | null>(null);

  const connectNWC = useCallback(async (url: string): Promise<void> => {
    // Parse nostr+walletconnect://<walletPubkey>?relay=<relay>&secret=<secret>
    const uri = url.replace('nostr+walletconnect://', 'https://nwc?pubkey=');
    const parsed = new URL(uri);
    const walletPubkey = parsed.searchParams.get('pubkey') || url.split('//')[1]?.split('?')[0];
    const relay = parsed.searchParams.get('relay');
    const secretHex = parsed.searchParams.get('secret');
    if (!walletPubkey || !relay || !secretHex) throw new Error('Invalid NWC URI');
    const secret = new Uint8Array(secretHex.match(/.{1,2}/g)!.map(b => parseInt(b, 16)));
    nwcRef.current = { walletPubkey, secret, relay };
    setNwcUrl(url);
    setNwcConnected(true);
    // Fetch initial balance
    try {
      const bal = await getWalletBalanceInternal({ walletPubkey, secret, relay });
  ...
  const nwcRequest = useCallback(async (
    conn: { walletPubkey: string; secret: Uint8Array; relay: string },
    method: string,
    params: Record<string, unknown>
  ): Promise<unknown> => {
    const { nip04 } = await import('nostr-tools');
    const { getPublicKey } = await import('nostr-tools');
    const appPubkey = getPublicKey(conn.secret);
    ...
    const signed = finalizeEvent(requestEvent as any, conn.secret);
    // Open relay, publish, wait for response
    return new Promise((resolve, reject) => {
      const ws = new WebSocket(conn.relay);
      const timeout = setTimeout(() => { ws.close(); reject(new Error('NWC timeout')); }, 15000);
      ws.onopen = () => {
      ...
      ws.onmessage = async (msg) => {
        ...
        if (data[0] === 'EVENT' && data[2]?.kind === 23195) {
          ...
          ws.close();
          try {
            const decrypted = await nip04.decrypt(conn.secret, conn.walletPubkey, data[2].content);
            const result = JSON.parse(decrypted);
            if (result.error) reject(new Error(result.error.message || 'NWC error'));
            else resolve(result.result);
          } catch (e) { reject(e); }
        }
      };
      ws.onerror = () => { clearTimeout(timeout); reject(new Error('NWC relay error')); };
    });
  }, []);

  const getWalletBalanceInternal = useCallback(async (
    conn: { walletPubkey: string; secret: Uint8Array; relay: string }
  ): Promise<number> => {
    const result = await nwcRequest(conn, 'get_balance', {}) as { balance: number };
    return Math.floor(result.balance / 1000); // msats -> sats
  }, [nwcRequest]);

  const getWalletBalance = useCallback(async (): Promise<number | null> => {
    if (!nwcRef.current) return null;
    try {
      const bal = await getWalletBalanceInternal(nwcRef.current);
      setWalletBalance(bal);
      return bal;
    } catch { return null; }
  ...
  const payInvoiceNWC = useCallback(async (invoice: string): Promise<void> => {
    ...
    await nwcRequest(nwcRef.current, 'pay_invoice', { invoice });
  }, [nwcRequest]);
  ...
  const loadRelayList = useCallback(async (pubKey: string): Promise<string[]> => {
    ...
    try {
      ...
      await relayStoreRef.current.add(events[0]);
      const relays = events[0].tags.filter(t => t[0] === 'r' && t[1]).map(t => t[1]);
  ...
  const loadBookmarks = useCallback(async (): Promise<string[]> => {
    ...
    const ids = events[0].tags.filter(t => t[0] === 'e').map(t => t[1]);
    const store = useNostrStore.getState();
