...
export const NostrProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const uploadFile = useCallback(async (file: File): Promise<string> => {
    ...
    // Upload to Blossom server via direct PUT request with auth header
    const pubkey = await signerRef.current.getPublicKey();
    const authEvent = await signerRef.current.signEvent({
      kind: 24242,
      content: 'Upload file',
      tags: [['t', 'upload'], ['expiration', String(Math.floor(Date.now() / 1000) + 3600)]],
      created_at: Math.floor(Date.now() / 1000),
    } as any);
  ...
  const connectNWC = useCallback(async (url: string): Promise<void> => {
    ...
    const wallet = WalletConnect.fromConnectionString(url);
